...
public abstract class TabHandler(public val tabName: String, private val page: PageHandler) : BaseHandler(page.basePath, page.templateBase) {
	public override fun attachTo(router: Router): TabHandler {
		router.route("${page.basePath}/$tabName").handler { ctx ->
			ctx.data<Boolean>().put("fromTab", true)
			if (!this.isFragment(ctx)) {
				ctx.reroute(this.page.basePath)
			}
